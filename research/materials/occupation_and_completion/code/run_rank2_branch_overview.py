#!/usr/bin/env python3
"""Short corrected-semantics rank-2 quotient branch overview."""
from __future__ import annotations
import json, subprocess, time
from pathlib import Path
ROOT=Path('research/research_record')
SPECS=ROOT/'workspace/data/quotient_branches/corrected_branch_specs.json'
CAP_BIN=str(ROOT/'workspace/data/quotient_dfs/rank2_cap5_n19.bin')
PAIR_BIN=str(ROOT/'workspace/data/quotient_dfs/rank2_cap7_first32.bin')
...
def main():
    import argparse
    ap=argparse.ArgumentParser(); ap.add_argument('--timeout',type=int,default=20); args=ap.parse_args()
    specs=json.loads(SPECS.read_text())['pivots']['rank2']['branches']
    out={'schema':'s0908_rank2_branch_overview_v1','timeout':args.timeout,'runs':[]}
    for b in specs:
        force=','.join(map(str,b['force'])); forbid=','.join(map(str,b['forbid']))
        for method in ['cap_only','pair32']:
            outfile=OUTDIR/f'rank2_overview_branch{b["branch"]}_{method}_{args.timeout}s.json'
            if method=='cap_only': cmd=[CAP_RUN,'--constraints',CAP_BIN,'--target','19','--force',force,'--timeout',str(args.timeout),'--output',str(outfile)]
            else: cmd=[PAIR_RUN,'--constraints',CAP_BIN,'--pair-rows',PAIR_BIN,'--target','19','--force',force,'--timeout',str(args.timeout),'--output',str(outfile)]
    ...
    path=OUTDIR/f'rank2_branch_overview_{args.timeout}s_summary.json'; path.write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('saved',path)

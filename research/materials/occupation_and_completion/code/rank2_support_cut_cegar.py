#!/usr/bin/env python3
"""analysis: find rank-2 quotient Wang survivors after transported lift support cuts.

This is an x-variable CEGAR loop: it keeps the exact quotient support problem
(no lift-bit variables), adds the 48 transported monotone cuts from the analysis
12-point lift obstruction, and searches for a fully quotient-Wang admissible
19-support that avoids all those cuts.  Such a support is genuinely different
from the first obstruction orbit and is a good target for the corrected all-row
lift-bit feasibility/minimization machinery.
"""
...
def load_cuts(path:Path):
    d=json.loads(path.read_text()); rhs=int(d['base_rhs']); cuts=[]
    for i,c in enumerate(d['unique_cuts']):
        cuts.append({'idx':i,'rhs':rhs,'points':[int(p) for p in c['cut_points']]})
    return d,cuts
...
def check_cuts(supp:set[int],cuts):
    ...
    for c in cuts:
        ...
        if occ>c['rhs']: out.append((c['idx'],c['rhs'],occ))
    return out

def solve_round(rows, active, cuts, prev_supports, time_limit, seed):
    ...
    st=solver.Solve(model); rec={'status':solver.StatusName(st),'wall_time':solver.WallTime(),'conflicts':solver.NumConflicts(),'branches':solver.NumBranches(),'active_constraints':len(active),'support_cuts':len(cuts)}
    if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
        rec['support']=sorted(p for p in range(1,256) if solver.Value(x[p]))
    return rec

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--initial-cap',type=int,default=3); ap.add_argument('--rounds',type=int,default=20); ap.add_argument('--time-per-round',type=int,default=120); ap.add_argument('--total-time',type=int,default=900); ap.add_argument('--seed',type=int,default=21000); ap.add_argument('--exclude-found',action='store_true')
    args=ap.parse_args(); t0=time.time(); rows=load_rows(QALL); cdata,cuts=load_cuts(TRANSPORT)
    ...
    for rnd in range(args.rounds):
        ...
        rec['full_quotient_violation_count']=len(viol); rec['full_quotient_violation_head']=[{'idx':i,'cap':c,'dim':d,'occ':o} for i,c,d,o in viol[:50]]; rec['support_cut_violation_count']=len(cviol); rec['support_cut_violation_head']=[{'idx':i,'rhs':rhs,'occ':occ} for i,rhs,occ in cviol[:20]]
        ...
        rounds.append(rec)
    out={'schema':'s0908_rank2_support_cut_cegar_v1','quotient_all_binary':str(QALL),'quotient_all_binary_sha256':sha(QALL),'transport_cut_manifest':str(TRANSPORT),'transport_cut_manifest_sha256':sha(TRANSPORT),'support_cut_count':len(cuts),'initial_cap':args.initial_cap,'rounds':rounds,'final_active_constraints':len(active),'fully_quotient_wang_admissible':any(r.get('fully_quotient_wang_admissible') for r in rounds),'final_status':rounds[-1]['status'] if rounds else 'no_rounds','elapsed_sec':time.time()-t0}
    path=OUTDIR/f'rank2_support_cut_cegar_cap{args.initial_cap}.json'
    path.write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
    print('saved',path,flush=True); print(json.dumps({'final_status':out['final_status'],'rounds':len(rounds),'fully_quotient_wang_admissible':out['fully_quotient_wang_admissible'],'elapsed_sec':out['elapsed_sec'],'final_active_constraints':len(active)},indent=2,sort_keys=True),flush=True)

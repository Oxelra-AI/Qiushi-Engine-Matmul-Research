...
struct Result {
    ...
    uint64_t pair_cover_checks=0, pair_cover_tests=0, pair_cover_prunes=0;
...
struct Searcher{
    ...
    const std::vector<Cut>& cuts; int target; double limit; int cover_min_depth; int cover_max_slack; int pair_cover_limit; bool use_mixed;
    ...
    Searcher(const std::vector<Row>& r,const std::vector<std::vector<uint32_t>>& p,const std::vector<Row>& cr,const std::vector<std::vector<uint32_t>>& cp,const std::vector<Cut>& c,int t,double lim,int cmd,int cms,int pcl):rows(r),pr(p),cov(cr),cpr(cp),cuts(c),target(t),limit(lim),cover_min_depth(cmd),cover_max_slack(cms),pair_cover_limit(pcl),use_mixed(!c.empty()),cnt(r.size(),0),ccnt(cr.size(),0),lhs(c.size(),0){}
    ...
    bool pair_cover_possible(int minp,int need){
        if(cov.empty() || pair_cover_limit<=1) return true;
        uint64_t fut[WORDS]; future_words(minp,fut); int avail=pop4(fut); int depth=(int)chosen.size(); int slack=avail-need;
        bool threshold = (need==0) || (depth>=cover_min_depth) || (cover_max_slack>=0 && slack<=cover_max_slack);
        if(!threshold) return true;
        int n=std::min<int>(pair_cover_limit,(int)cov.size());
        std::vector<std::array<uint64_t,WORDS>> fm(n); std::vector<int> fin(n), u(n);
        for(int i=0;i<n;i++){
            u[i]=(int)cov[i].cap-(int)ccnt[i];
            for(int w=0;w<WORDS;w++) fm[i][w]=fut[w]&cov[i].word[w];
            fin[i]=__builtin_popcountll(fm[i][0])+__builtin_popcountll(fm[i][1])+__builtin_popcountll(fm[i][2])+__builtin_popcountll(fm[i][3]);
        }
        res.pair_cover_checks++;
        for(int i=0;i<n;i++){
            for(int j=i+1;j<n;j++){
                res.pair_cover_tests++;
                int ui=u[i], uj=u[j];
                if(ui<0 || uj<0){ res.pair_cover_prunes++; return false; }
                int both=0; for(int w=0;w<WORDS;w++) both += __builtin_popcountll(fm[i][w]&fm[j][w]);
                int ionly=fin[i]-both, jonly=fin[j]-both, outside=avail-(both+ionly+jonly);
                int max_inside=0; int uboth=std::min(both,std::min(ui,uj));
                for(int x=0;x<=uboth;x++){
                    int val=x+std::min(ionly,ui-x)+std::min(jonly,uj-x);
                    if(val>max_inside) max_inside=val;
                }
                int bound=outside+max_inside;
                if(bound<need){ res.pair_cover_prunes++; return false; }
            }
        }
        return true;
    }
    ...
    void dfs(int minp){
        ...
        int d=(int)chosen.size(); if(d>res.max_depth) res.max_depth=d; if(d==target){ if(!cover_possible(MAXP+1,0)) return; if(!pair_cover_possible(MAXP+1,0)) return; if(!mixed_possible(MAXP+1,0)) return; res.solutions++; if(res.sol_head.size()<5){ auto s=chosen; std::sort(s.begin(),s.end()); res.sol_head.push_back(s); } return; }
        int need=target-d; int avail=avail_from(minp); if(avail<need){ res.global_shortage++; return; } if(!cover_possible(minp,need)) return; if(!pair_cover_possible(minp,need)) return; if(!mixed_possible(minp,need)) return;

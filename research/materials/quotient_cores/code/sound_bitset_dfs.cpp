// analysis: optimized sound bitset DFS for E11 Wang upper bounds, optionally with
// mixed-source lower cuts.
//
// Mathematical semantics:
//   Wang row R: |S cap R| <= cap.  When selected count reaches cap, future
//   points in R are removed.  Unused capacity does not require filling.
//   Mixed row: sum c_p x_p >= K.  At a partial node, reject only when even the
//   best future points still in the current domain cannot reach K.
//
// State restoration is transactional: rows are prechecked before incrementing;
// exactly incremented counters are undone; the 256-point domain bitset is saved
// and restored at each branch.
...
struct Result {
    ...
    uint64_t calls=0, candidate_tests=0, overcap_rejects=0, global_shortage=0, mixed_prunes=0, mixed_checks=0;
    uint64_t saturated_rows=0, saturation_removed_points=0, initial_cap0_removed=0, solutions=0;
    std::array<uint64_t,32> nodes{}; std::array<uint64_t,32> mixed_depth{}; std::vector<std::vector<int>> sol_head;
};
...
static bool load_rows(const std::string& path, std::vector<Row>& rows){
    ...
    char magic[8]; in.read(magic,8); if(!in || std::memcmp(magic,"E11WANG1",8)!=0){ std::cerr<<"bad row magic\n"; return false; }
...
struct Searcher{
    const std::vector<Row>& rows; const std::vector<std::vector<uint32_t>>& pr; const std::vector<Cut>& cuts; int target; double limit; bool use_mixed;
    std::chrono::steady_clock::time_point start; std::vector<uint8_t> cnt; std::vector<int16_t> lhs; uint64_t dom[WORDS]; std::vector<int> chosen; Result res;
    Searcher(const std::vector<Row>& r,const std::vector<std::vector<uint32_t>>& p,const std::vector<Cut>& c,int t,double lim):rows(r),pr(p),cuts(c),target(t),limit(lim),use_mixed(!c.empty()),cnt(r.size(),0),lhs(c.size(),0){ }
    ...
    int avail_from(int minp) const {
        int c=0; for(int p=minp;p<=MAXP;p++) if(in_dom(p)) c++; return c;
    }
    void future_words(int minp, uint64_t out[WORDS]) const {
        for(int w=0;w<WORDS;w++) out[w]=dom[w];
        if(minp<=1) return;
        for(int p=1;p<minp && p<=MAXP;p++) clrbit4(out,p);
    }
    bool mixed_possible(int minp,int need){
        ...
        for(size_t i=0;i<cuts.size();i++){
            ...
            if(ub < cuts[i].K){ res.mixed_prunes++; int d=chosen.size(); if(d>=0 && d<(int)res.mixed_depth.size()) res.mixed_depth[d]++; return false; }
        }
        return true;
    }
    bool try_select(int q, std::vector<uint32_t>& touched, uint64_t olddom[WORDS]){
        ...
        remove_dom(q); for(uint32_t ri:pr[q]){ cnt[ri]++; touched.push_back(ri); }
    ...
    void undo_select(int q,const std::vector<uint32_t>& touched,const uint64_t olddom[WORDS]){
        for(size_t i=0;i<cuts.size();i++) lhs[i] -= cuts[i].coeff[q]; for(auto it=touched.rbegin();it!=touched.rend();++it) cnt[*it]--; for(int w=0;w<WORDS;w++) dom[w]=olddom[w];
    }
    bool force_points(const std::vector<int>& force){
        for(int p:force){ if(!in_dom(p)){ res.forced_infeasible=true; return false; } std::vector<uint32_t> touched; uint64_t old[WORDS]; bool ok=try_select(p,touched,old); if(!ok){ res.forced_infeasible=true; return false; } chosen.push_back(p); }
        return true;
    }
    void dfs(int minp){
        if(res.timeout) return; if((res.calls++ & 0x3FFFULL)==0 && timed()){ res.timeout=true; return; }
        int d=chosen.size(); if(d>res.max_depth) res.max_depth=d; if(d==target){ if(!mixed_possible(MAXP+1,0)) return; res.solutions++; if(res.sol_head.size()<5){ auto s=chosen; std::sort(s.begin(),s.end()); res.sol_head.push_back(s); } return; }
        int need=target-d; if(avail_from(minp)<need){ res.global_shortage++; return; } if(!mixed_possible(minp,need)) return;
        for(int q=minp;q<=MAXP;q++){
            if(MAXP-q+1<need) break; if(!in_dom(q)) continue; if(avail_from(q)<need) break;
            std::vector<uint32_t> touched; uint64_t old[WORDS]; bool ok=try_select(q,touched,old);
            if(ok){ chosen.push_back(q); int nd=chosen.size(); if(nd<(int)res.nodes.size()) res.nodes[nd]++; if(mixed_possible(q+1,target-nd)) dfs(q+1); chosen.pop_back(); undo_select(q,touched,old); }
            if(res.timeout) return; if(res.solutions>100) return;
        }
    }
    Result run(const uint64_t forbidden[WORDS], const std::vector<int>& force, int min_after, int branch, int rep){
        res=Result(); res.branch=branch; res.rep=rep; res.target=target; res.nodes.fill(0); res.mixed_depth.fill(0); res.nodes[0]=1; std::fill(cnt.begin(),cnt.end(),0); std::fill(lhs.begin(),lhs.end(),0); chosen.clear(); full_domain(); for(int w=0;w<WORDS;w++) dom[w] &= ~forbidden[w];
        ...
        start=std::chrono::steady_clock::now(); bool ok=force_points(force); if(ok){ int d=chosen.size(); if(d<(int)res.nodes.size()) res.nodes[d]=std::max<uint64_t>(res.nodes[d],1); dfs(min_after); }
        res.elapsed_sec=std::chrono::duration<double>(std::chrono::steady_clock::now()-start).count(); res.complete=!res.timeout; return res;
    }
};
static std::string rjson(const Result& r){ std::ostringstream o; o<<"{\"branch\":"<<r.branch<<",\"rep\":"<<r.rep<<",\"target\":"<<r.target<<",\"elapsed_sec\":"<<std::fixed<<std::setprecision(6)<<r.elapsed_sec<<",\"timeout\":"<<(r.timeout?"true":"false")<<",\"complete\":"<<(r.complete?"true":"false")<<",\"forced_infeasible\":"<<(r.forced_infeasible?"true":"false")<<",\"recursive_calls\":"<<r.calls<<",\"candidate_tests\":"<<r.candidate_tests<<",\"overcap_rejects\":"<<r.overcap_rejects<<",\"global_shortage\":"<<r.global_shortage<<",\"mixed_prunes\":"<<r.mixed_prunes<<",\"mixed_checks\":"<<r.mixed_checks<<",\"saturated_rows\":"<<r.saturated_rows<<",\"saturation_removed_points\":"<<r.saturation_removed_points<<",\"initial_cap0_removed\":"<<r.initial_cap0_removed<<",\"solutions\":"<<r.solutions<<",\"max_depth\":"<<r.max_depth<<",\"node_count_by_depth\":"<<arr_json(r.nodes,std::min(31,r.target))<<",\"mixed_prune_depth\":"<<arr_json(r.mixed_depth,std::min(31,r.target))<<",\"solutions_head\":"<<sols_json(r.sol_head)<<"}"; return o.str(); }

int main(int argc,char**argv){
    std::string cons,cuts,outp,force_s,exclude_s; int target=19, branch=-2; double lim=60.0;
    for(int i=1;i<argc;i++){ std::string a=argv[i]; if(a=="--constraints"&&i+1<argc) cons=argv[++i]; else if(a=="--cuts"&&i+1<argc) cuts=argv[++i]; else if(a=="--output"&&i+1<argc) outp=argv[++i]; else if(a=="--target"&&i+1<argc) target=std::stoi(argv[++i]); else if(a=="--timeout"&&i+1<argc) lim=std::stod(argv[++i]); else if(a=="--branch"&&i+1<argc) branch=std::stoi(argv[++i]); else if(a=="--all-branches") branch=-1; else if(a=="--force"&&i+1<argc) force_s=argv[++i]; else if(a=="--exclude"&&i+1<argc) exclude_s=argv[++i]; else { std::cerr<<"bad arg "<<a<<"\n"; return 2; } }
    ...
    std::vector<Row> rows; if(!load_rows(cons,rows)) return 1; std::vector<Cut> cutv; if(!load_cuts(cuts,cutv)) return 1; auto pr=point_rows(rows); std::vector<int> force=parse_list(force_s), exclude=parse_list(exclude_s); uint64_t forbidden[WORDS]={0,0,0,0}; for(int p:exclude) setbit4(forbidden,p);
    std::vector<Result> results; if(branch>=-1){ for(auto& br:branches()){ if(branch>=0 && br.id!=branch) continue; uint64_t forb[WORDS]; for(int w=0;w<WORDS;w++) forb[w]=forbidden[w]|br.forbid[w]; auto f=force; f.push_back(br.rep); std::sort(f.begin(),f.end()); f.erase(std::unique(f.begin(),f.end()),f.end()); Searcher S(rows,pr,cutv,target,lim); Result r=S.run(forb,f,br.rep+1,br.id,br.rep); results.push_back(r); std::cout<<rjson(r)<<"\n"; std::cout.flush(); } }
    else { Searcher S(rows,pr,cutv,target,lim); Result r=S.run(forbidden,force,1,-1,0); results.push_back(r); std::cout<<rjson(r)<<"\n"; }
    bool all=true; uint64_t sol=0,calls=0,tests=0,over=0,shortage=0,mp=0,mchk=0,sat=0,srem=0,init0=0; int md=0; std::array<uint64_t,32> nodes{}; nodes.fill(0); std::array<uint64_t,32> mpd{}; mpd.fill(0);
    for(auto&r:results){ all&=r.complete; sol+=r.solutions; calls+=r.calls; tests+=r.candidate_tests; over+=r.overcap_rejects; shortage+=r.global_shortage; mp+=r.mixed_prunes; mchk+=r.mixed_checks; sat+=r.saturated_rows; srem+=r.saturation_removed_points; init0+=r.initial_cap0_removed; md=std::max(md,r.max_depth); for(int i=0;i<32;i++){ nodes[i]+=r.nodes[i]; mpd[i]+=r.mixed_depth[i]; } }
    std::ostringstream out; out<<"{\n  \"schema\": \"s0908_sound_bitset_dfs_v1\",\n  \"semantics\": \"Wang upper-bound saturation with bitset transactional state; optional mixed lower-cut domain-aware future upper bound\",\n  \"constraints_path\": \""<<cons<<"\",\n  \"cut_path\": \""<<cuts<<"\",\n  \"constraint_count\": "<<rows.size()<<",\n  \"mixed_cut_count\": "<<cutv.size()<<",\n  \"target\": "<<target<<",\n  \"time_limit_per_run_sec\": "<<lim<<",\n  \"all_complete\": "<<(all?"true":"false")<<",\n  \"total_solutions\": "<<sol<<",\n  \"global_max_depth\": "<<md<<",\n  \"total_recursive_calls\": "<<calls<<",\n  \"total_candidate_tests\": "<<tests<<",\n  \"total_overcap_rejects\": "<<over<<",\n  \"total_global_shortage\": "<<shortage<<",\n  \"total_mixed_prunes\": "<<mp<<",\n  \"total_mixed_checks\": "<<mchk<<",\n  \"total_saturated_rows\": "<<sat<<",\n  \"total_saturation_removed_points\": "<<srem<<",\n  \"total_initial_cap0_removed\": "<<init0<<",\n  \"total_node_count_by_depth\": "<<arr_json(nodes,std::min(31,target))<<",\n  \"total_mixed_prune_depth\": "<<arr_json(mpd,std::min(31,target))<<",\n  \"results\": [\n"; for(size_t i=0;i<results.size();i++){ out<<"    "<<rjson(results[i]); if(i+1<results.size()) out<<","; out<<"\n"; } out<<"  ]\n}\n";

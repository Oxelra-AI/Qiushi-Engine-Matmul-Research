...
struct Searcher{
    ...
    void dfs(int minp){
        ...
        if(d==target){
            ...
            // Check pair margins at terminal
            for(size_t pi=0;pi<pairs.size();pi++) if(pair_margin[pi]<0) return;
        ...
        if(!propagate_pairs(minp, removed, forced_mand)) return;
        ...
        for(int w=0;w<WORDS;w++) dom[w]=prop_dom[w]; // already prop_dom from save
        // Need to restore to ORIGINAL pre-propagation domain
        for(int p:removed) setbit4(dom,p);
        for(size_t i=0;i<pairs.size();i++) pair_margin[i]=saved_margins[i];
        for(int p:forced_mand) clrbit4(mandatory,p);
    }

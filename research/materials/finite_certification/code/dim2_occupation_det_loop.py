#!/usr/bin/env python3
"""Bounded occupation-support enumeration plus trace and det fixed-A completion.

For a analysis terminal dim-2 quotient orbit W, enumerate length-18 occupation-
admissible A-multisets in F2^7.  For each multiset, first run the relaxed
tight-trace test.  If trace is inconsistent, block the multiset.  If trace
passes, run the inverse-determination fixed-A quotient SAT oracle.  SAT with
zero residual would be an actual quotient decomposition; UNSAT blocks only the
current multiset; UNKNOWN stops because coverage cannot proceed reliably.

This is not a final proof unless the loop exhausts the support system with all
SAT calls UNSAT and those UNSATs are independently replayed/certified.  It is
an execution probe for whether the remaining dim-2 orbit quotients have a small
enumerable support frontier.
"""
from __future__ import annotations

import argparse
import json
import pickle
import sys
import time
from collections import Counter
from pathlib import Path

try:
    from ortools.sat.python import cp_model  # type: ignore
except Exception:  # pragma: no cover
    cp_model = None

SCRIPT = Path(__file__).resolve().parent
if str(SCRIPT) not in sys.path:
    sys.path.insert(0, str(SCRIPT))

from node_local_occupation_closure import LUT_PATH, node_constraints  # type: ignore
from dim2_quotient_det import orbit_basis, trace_analyze, det_solve  # type: ignore


def load_lut():
    with LUT_PATH.open("rb") as f:
        d = pickle.load(f)
    return d["lut"] if isinstance(d, dict) and "lut" in d else d


def rows_for_orbit(orbit_index: int, target: int):
    W = orbit_basis(orbit_index)
    lut = load_lut()
    rows, meta = node_constraints(W, lut, target)
    usable = []
    singleton_caps = {}
    for mask, cap in rows:
        pts = [i + 1 for i in range((1 << 7) - 1) if (int(mask) >> i) & 1]
        if len(pts) == 1:
            singleton_caps[pts[0]] = min(singleton_caps.get(pts[0], target), int(cap))
        if int(cap) < target:
            usable.append((pts, int(cap), int(mask)))
    meta = dict(meta)
    meta.update({
        "usable_rows_integer": len(usable),
        "singleton_caps_count": len(singleton_caps),
        "singleton_cap_min": int(min(singleton_caps.values())) if singleton_caps else None,
        "singleton_cap_max": int(max(singleton_caps.values())) if singleton_caps else None,
        "singleton_cap_hist": dict(sorted((int(k), int(v)) for k, v in Counter(singleton_caps.values()).items())),
    })
    return W, usable, meta


def solve_round(rows, target: int, forbidden: list[list[int]], time_limit: float, workers: int, seed: int):
    if cp_model is None:
        return {"status": "NO_OR_TOOLS", "solution_vec": None, "elapsed_sec": 0.0, "conflicts": 0, "branches": 0}
    model = cp_model.CpModel()
    xs = [model.NewIntVar(0, target, f"x_{q}") for q in range(1, 128)]
    model.Add(sum(xs) == target)
    for pts, cap, _mask in rows:
        if cap < 0:
            model.Add(0 <= -1)
        else:
            model.Add(sum(xs[q - 1] for q in pts) <= cap)
    for vec in forbidden:
        model.AddForbiddenAssignments(xs, [tuple(int(v) for v in vec)])
    solver = cp_model.CpSolver()
    solver.parameters.max_time_in_seconds = float(time_limit)
    solver.parameters.num_search_workers = int(workers)
    solver.parameters.random_seed = int(seed)
    t0 = time.time()
    st = solver.Solve(model)
    elapsed = time.time() - t0
    name = solver.StatusName(st)
    vec = None
    if st in (cp_model.FEASIBLE, cp_model.OPTIMAL):
        vec = [int(solver.Value(x)) for x in xs]
    return {
        "status": name,
        "solution_vec": vec,
        "elapsed_sec": elapsed,
        "conflicts": int(solver.NumConflicts()),
        "branches": int(solver.NumBranches()),
    }


def vec_to_multiset(vec: list[int]) -> list[int]:
    out = []
    for i, m in enumerate(vec, start=1):
        out.extend([i] * int(m))
    return out


def vec_summary(vec: list[int]) -> dict:
    nz = [(i + 1, int(v)) for i, v in enumerate(vec) if v]
    return {
        "support_nonzero_count": len(nz),
        "max_multiplicity": max((v for _i, v in nz), default=0),
        "solution_head": nz[:127],
        "solution_weight_check": sum(v for _i, v in nz),
    }


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--orbit-index", type=int, required=True)
    ap.add_argument("--target", type=int, default=18)
    ap.add_argument("--max-solutions", type=int, default=200)
    ap.add_argument("--total-time-limit", type=float, default=1800.0)
    ap.add_argument("--time-limit-per-round", type=float, default=20.0)
    ap.add_argument("--workers", type=int, default=8)
    ap.add_argument("--det-timeout", type=int, default=60)
    ap.add_argument("--solver", default="cadical", choices=["cadical", "kissat", "cryptominisat5"])
    ap.add_argument("--mode", default="b2c", choices=["b2c", "c2b", "both"])
    ap.add_argument("--out", type=Path, required=True)
    args = ap.parse_args()
    t0 = time.time()
    W, rows, meta = rows_for_orbit(args.orbit_index, args.target)
    det_dir = args.out.parent / "det_rounds"
    forbidden: list[list[int]] = []
    rounds = []
    counts = Counter()
    final_status = None
    for r in range(args.max_solutions):
        if time.time() - t0 > args.total_time_limit:
            final_status = "TOTAL_TIME_LIMIT"
            break
        sol = solve_round(rows, args.target, forbidden, args.time_limit_per_round, args.workers, 31000 + 1009 * r + args.orbit_index)
        final_status = sol["status"]
        rec = {"round": r, "solve": {k: v for k, v in sol.items() if k != "solution_vec"}}
        vec = sol.get("solution_vec")
        if vec is None:
            rounds.append(rec)
            break
        multiset = vec_to_multiset(vec)
        rec["support"] = vec_summary(vec)
        tr = trace_analyze(W, multiset)
        rec["trace"] = {
            "full_trace_consistent": bool(tr["full_trace_consistent"]),
            "tensor_consistent": bool(tr["tensor_consistent"]),
            "full_rank_aug": tr["full_rank_aug"],
            "tight_lambda_count": tr["tight_lambda_count"],
            "tight_rank_hist": tr["tight_rank_hist"],
            "elapsed_sec": tr["elapsed_sec"],
        }
        if not tr["full_trace_consistent"]:
            counts["trace_reject"] += 1
            forbidden.append(vec)
            rounds.append(rec)
            continue
        counts["trace_pass"] += 1
        det = det_solve(W, multiset, det_dir / f"round_{r:04d}", f"orbit{args.orbit_index}_round{r:04d}_det", args.det_timeout, args.solver, args.mode)
        rec["det"] = {
            "sat": det["sat"],
            "returncode": det["returncode"],
            "elapsed_sec": det["elapsed_sec"],
            "result_path": str(det_dir / f"round_{r:04d}" / f"orbit{args.orbit_index}_round{r:04d}_det_result.json"),
            "cnf_vars": det["encoding"]["cnf_vars"],
            "cnf_clauses": det["encoding"]["cnf_clauses"],
            "tight_lambda_count": det["encoding"]["tight_lambda_count"],
            "tight_rank_hist": det["encoding"]["tight_rank_hist"],
            "verification_diff": det.get("verification_diff"),
            "witness_path": det.get("witness_path"),
        }
        rounds.append(rec)
        if det["sat"] is True and det.get("verification_diff") == 0:
            counts["sat_witness"] += 1
            final_status = "SAT_WITNESS"
            break
        if det["sat"] is False:
            counts["det_unsat"] += 1
            forbidden.append(vec)
            continue
        counts["det_unknown"] += 1
        final_status = "DET_UNKNOWN"
        break
    else:
        final_status = "MAX_SOLUTIONS"
    meaning = final_status
    if final_status == "INFEASIBLE":
        meaning = "occupation_exhausted_after_blocking"
    out = {
        "schema": "dim2_occupation_det_loop_v1",
        "orbit_index": int(args.orbit_index),
        "W_basis": [int(x) for x in W],
        "target": int(args.target),
        "row_meta": meta,
        "parameters": {
            "max_solutions": int(args.max_solutions),
            "total_time_limit": float(args.total_time_limit),
            "time_limit_per_round": float(args.time_limit_per_round),
            "workers": int(args.workers),
            "det_timeout": int(args.det_timeout),
            "solver": args.solver,
            "mode": args.mode,
        },
        "counts": dict(counts),
        "blocked_vectors": len(forbidden),
        "rounds_completed": len(rounds),
        "final_status": final_status,
        "status_meaning": meaning,
        "rounds": rounds,
        "elapsed_sec": time.time() - t0,
    }
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(out, indent=2, sort_keys=True) + "\n")
    print(json.dumps({
        "out": str(args.out),
        "orbit_index": args.orbit_index,
        "counts": dict(counts),
        "blocked_vectors": len(forbidden),
        "rounds_completed": len(rounds),
        "final_status": final_status,
        "status_meaning": meaning,
        "elapsed_sec": out["elapsed_sec"],
    }, indent=2, sort_keys=True))


if __name__ == "__main__":
    main()

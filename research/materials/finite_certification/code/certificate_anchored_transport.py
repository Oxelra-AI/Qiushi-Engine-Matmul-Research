...
analysis exposed transports from cached orbit representatives to the subspaces
...
TRANSPARENT_BATCH = Path('data/transparent_unary_certs_484_491/batch_transparent_unary_dim2_certs_manifest.json')
LIFTING_BATCH = Path('data/transparent_lifting_batch/batch_transparent_lifting_verifier_manifest.json')
OUT = Path('data/anchored_transport/certificate_anchored_transport.json')
JSONL = Path('data/anchored_transport/dim2_core_row_anchored_witnesses.jsonl')
...
def parse_certificate_records(path: Path) -> Tuple[Dict[str, object], List[Dict]]:
    ...
    for block in text.split('constrained_tensors {')[1:]:
        ...
        # this conservative slice is the same robust strategy used in the earlier research

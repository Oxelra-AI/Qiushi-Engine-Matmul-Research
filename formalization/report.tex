...
are proved within Lean, rather than assumed from an external solver. The
...
\end{equation}
The lower bound rules out all such algorithms using at most 20 products; it
does not construct a 21-product algorithm. The upper bound is supplied by an
...
\end{lstlisting}
This display uses ASCII equivalents for Lean notation. Its quantification
is over every input matrix, not a set of test inputs. The tensor identity
and the bilinear algorithm statement are related by proved equivalences.
Neither this theorem nor \texttt{rank\_ge\_21} assumes any finite quotient bound.
...
they are not isolated Boolean calculations with assumed interpretations.
...
Certificate generation need not be trusted to announce correctness, since
its output must supply a valid proof term.
...
core witness; it does not assert that such a witness exists. Statements
...
Thus the finite premises do not enter as added axioms. The verification
uses the official Lean kernel; it is not a second implementation of that
kernel. \texttt{verification-results.json} records the checked inputs and
outcomes, and \texttt{verification.json} specifies the terminal declarations.
...
finite computation. Their contribution is more than an additional executable
check of one result: they turn the structural mechanism and its finite
prerequisites into explicit mathematical components whose statements and
dependencies can be inspected and extended. The accompanying research

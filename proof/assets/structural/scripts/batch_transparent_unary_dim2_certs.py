...
def main() -> None:
    ...
    ap.add_argument('--cores-root', type=Path, default=Path('assets/structural/data/dim2_caplimited_cores_484_491'))
    ap.add_argument('--outdir', type=Path, default=Path('assets/structural/data/transparent_unary_certs_484_491'))
    ...
    for idx in [int(x) for x in args.indices.split(',') if x.strip()]:
        ...
        cmd = [
            'python', 'assets/structural/scripts/transparent_unary_dim2_cnf.py',
